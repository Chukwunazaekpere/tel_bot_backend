...
const depositController = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        const data = await axios.get(`${process.env.BASE_URL}/receive?xpub=${process.env.X_PUB_KEY}&callback=${process.env.CALLBACK_URL}&key=${process.env.BLOCKCHAIN_API_KEY}`, {
                headers: {
