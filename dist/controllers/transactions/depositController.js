...
const blockchain_network = "BITCOIN";
const depositController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    console.log("Data: deposit", `${process.env.BASE_URL}/receive?xpub=${process.env.X_PUB_KEY}&callback=${process.env.CALLBACK_URL}&key=${process.env.BLOCKCHAIN_API_KEY}`);
    console.log('CHAINED_API_KEY: ', process.env.CHAINED_API_KEY);
    try {
        const data = yield axios_1.default.get(`${process.env.BASE_URL}/receive?xpub=${process.env.X_PUB_KEY}&callback=${process.env.CALLBACK_URL}&key=${process.env.BLOCKCHAIN_API_KEY}`, {
            headers: {
                'X-API-Token': `${process.env.CHAINED_API_KEY}`,
        ...
        // const options = {
        // method: 'POST',
        // headers: {Authorization: `Bearer ` + `${process.env.BTC_KEY}`},
